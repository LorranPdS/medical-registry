...
public class UsuarioService{
    ...
    public void cadastrar(UsuarioRequest request){
        ...
        for (ValidacaoUsuario validacao : validacoes) {
            validacao.validar(request);
        }
//        validacoes.forEach(validacao -> validacao.validar(request)); // Outra forma de fazer a iteração acima

        log.info("Cadeia de validações finalizada com sucesso");
        log.info("Simulando alguma outra rotina para salvar Usuario [{}] na base de dados...", request.nome());
    }
}
